...
##### Taxa will need to be named as a unique Taxa ID and put as rownames
st_rows<- functions %>%
  ...
  group_by (sample_name, Subsystem) %>%
  mutate (new_abund = sum(abund), new_abund = round(new_abund, 0)) %>%
  dplyr::select (-abund) %>%
  distinct(sample_name, Subsystem, .keep_all = TRUE) %>%
  ungroup () %>%
  #mutate (otu = paste0("sp",otu)) %>%
  #dplyr::select (-all_of(which_group)) %>%
...
func_saw <- st_rows %>%
  dplyr::select(c(ss_id, Order, Family, Genus, Species)) #Level_1, Level_2, Level_3, Subsystem
...
st_rows <- st_rows %>%
  ...
  dplyr::select(-c(Order, Family, Genus, Species))
...
res_out <- do.call(cbind, res_out)
res_sig <- res_out %>%
  rownames_to_column ("subsystems") %>%
  arrange(-lfc.environmentwater) %>%
  mutate (sig = ifelse(q_val.environmentwater <= 0.05, "sig","nonsig")) %>%
  mutate(abs_lfc = abs(lfc.environmentwater)) %>%
  left_join (func_saw %>% rownames_to_column ("ss_id"), by = c("subsystems"="Species"))
...
list_sig_family <- res_sig %>% ### From ACOMBC analysis above
  filter(sig == "sig")
list_sf <- as.character(list_sig_family$Family)
...
cor_func <- functions %>%
  ...
  distinct(sample_name, Level_3, .keep_all = TRUE) %>%
  #rename(tran = 4) %>%
  dplyr::select (-c(Level_1, Level_2, Subsystem, to_keep, prop, level_sum, func_name, environment)) %>%
  arrange(sample_name) %>%
  pivot_wider (names_from = Level_3, values_from = "tran_level", values_fill = 0) %>%
  column_to_rownames ("sample_name")
df2 <- cor_func
...
heatmap(cor_wide_mat, scale = "none")
